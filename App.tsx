...
const App: React.FC = () => {
    ...
    const handleRegistrationSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 2. Proceed to payment simulation
            const prices = {
                chercheur: { amount: 15000, currency: 'EUR', equivalent: '~98 000 XOF' },
                etudiant: { amount: 7500, currency: 'EUR', equivalent: '~49 000 XOF' },
            };
            const selectedPrice = prices[registration.participantType as keyof typeof prices];
            
            alert(
                `--- Inscription enregistrée ! Redirection vers Paystack ---\n\n` +
                `Utilisateur: ${registration.email}\n` +
                `Type: ${registration.participantType}\n` +
                `Montant: ${selectedPrice.amount / 100} ${selectedPrice.currency}\n\n` +
                `NOTE TECHNIQUE :\nAprès un paiement réussi, Paystack appellera une URL "webhook" sur votre serveur. Ce webhook est responsable de :\n` +
                `1. Vérifier la transaction.\n` +
                `2. Mettre à jour le statut de paiement dans Firestore (de 'pending' à 'completed').\n` +
                `3. Envoyer un e-mail de confirmation au participant.`
            );
